...
function injectBaseSEOMeta() {
  ...
  const metas = [
    [
      "name",
      "description",
      "Descubrí emprendimientos independientes con identidad propia. Gastronomía, deco, regalos, moda y más.",
    ],
    ["property", "og:title", "Viko — Directorio de Emprendimientos"],
    [
      "property",
      "og:description",
      "Productos, servicios y emprendimientos seleccionados en un solo lugar.",
    ],
...
function getPlanLabel(plan) {
  if (plan === "premium")
    return { text: "Premium", bg: "#1A1814", color: "#fff" };
  if (plan === "featured")
    return { text: "Destacado", bg: "#C9A84C", color: "#fff" };
...
function showQRModal(empId) {
  ...
  overlay.innerHTML = `
    <div style="background:#FAFAF7;border-radius:24px;padding:40px 32px;text-align:center;max-width:360px;width:100%;position:relative;box-shadow:0 24px 64px rgba(26,24,20,0.2);">
      ...
      <div style="font-size:13px;color:#7A756A;margin-bottom:24px;">QR de tu ficha en Viko</div>
...
function renderCard(e) {
  const img =
    e.images && e.images.length > 0 ? e.images[0] : "img/placeholder.jpg";
  const waMensaje = `Hola ${e.nombre}! Vi tu ficha en Viko.`;
  return `
      ...
      <div class="card-body">
        ...
        <div class="card-footer">
          <div class="card-links">
            ...
            ${
              e.web
                ? `<a class="card-link-btn" href="${e.web}"
...
function sortCards(val) {
  ...
  if (val === "az")
    ...
    cards.sort(
      (a, b) => PLAN_ORDER[a.dataset.plan] - PLAN_ORDER[b.dataset.plan],
    );
...
function openModal(id) {
  ...
  document.getElementById("carousel-dots").innerHTML = currentImages
    .map(
      (_, i) =>
  ...
  const waPersonalizado = buildWhatsAppUrl(
    e.whatsapp,
    `Hola ${e.nombre}! Vi tu perfil en Viko y me gustaría recibir atención personalizada 😊`,
  );
...
function updateCarouselLabel() {
  document.getElementById("carousel-label").textContent =
    SLIDE_LABELS[carouselIndex] || `Foto ${carouselIndex + 1}`;
}
function carouselNext() {
  carouselIndex = (carouselIndex + 1) % currentImages.length;
  updateCarousel();
}
function carouselPrev() {
  carouselIndex =
    (carouselIndex - 1 + currentImages.length) % currentImages.length;
  updateCarousel();
}
function goSlide(i) {
  carouselIndex = i;
...
function openWhatsApp() {
  window.open(
    buildWhatsAppUrl("5492254414211", "Hola! Quiero publicar en Viko."),
    "_blank",
  );
}
...
setTimeout(
  () =>
    animateCount(document.getElementById("stat-total"), EMPRENDIMIENTOS.length),
  600,
);
